...
| --- | --- | --- | --- |
| CLI 엔트리포인트 | `<runtime-root>/.local/bin/hermes` → `hermes_cli.main` | `which hermes`, `import hermes` 경로 확인 | 부분 불일치 (`import hermes` 가능한 top-level 모듈 없음) |
| 패키지 버전 | `hermes-agent 0.8.0` | live runtime 확인 요구 | 없음 |
| 플러그인 로딩 경로 | 일반 플러그인: `<hermes-home>/plugins/`, 프로젝트 플러그인: `./.hermes/plugins/`(env opt-in), pip entrypoint: `hermes_agent.plugins` | plugin loading path 확인 | 없음 |
| 메모리 provider 실제 위치 | `site-packages/plugins/memory/*`; `<hermes-home>/plugins/memory/`는 없음 | `<hermes-home>/plugins/memory/` 기존 provider 엔트리포인트 학습 | 불일치 |
...
| 활성 memory provider | `<runtime-root>/.hermes/config.yaml`의 `memory.provider`가 빈 문자열 → built-in only | provider 확인 | 없음 |
...
| MCP 설정 위치 | `<hermes-home>/config.yaml`의 `mcp_servers` 사용 | `<hermes-home>/config/mcp.yaml` 또는 `~/.config/mcp/` 확인 | 부분 불일치 |
...
| direct auth fallback location | env(`NOTION_API_KEY` 등) 또는 `<runtime-root>/.openclaw/openclaw.json#skills.entries.notion.apiKey` | 토큰 위치만 기록 | 부분 일치 |
...
| active auth state location | `<runtime-root>/.hermes/auth.json` (`active_provider: openai-codex`) | API key/location 확인 | 없음 |
| 보조 credential pool | `<runtime-root>/.hermes/auth.json`, `<runtime-root>/.hermes/.env`, `config.yaml` | API key/location 확인 | 없음 |
...
| stale alt config | `<runtime-root>/.hermes/config.toml`에 별도 custom provider 흔적 존재, 현재 active runtime과 불일치 | live runtime 우선 | 불일치 |
...
| 유력 vault 경로 | `<runtime-root>/obsidian/<vault-name>/<vault-name>` (실파일 다수 존재) | vault path 확정 필요 | 미확정(추정) |
...
| 항목 | 실측값 | 사양서 기재값 | 불일치 여부 |
| --- | --- | --- | --- |
| vault path | 유력 후보 `<runtime-root>/obsidian/<vault-name>/<vault-name>` | vault path 확정 | 미확정(추정) |
...
- `vault_root` proposal: `<runtime-root>/obsidian/<vault-name>/<vault-name>` **단, live config 근거는 없어서 사용자 확인 필요**

## 사양 대비 불일치 요약

1. Hermes runtime에는 top-level `hermes` import가 없고, memory provider는 `<hermes-home>/plugins/memory/`가 아니라 site-packages `plugins/memory/*`에 존재한다.
...
5. `<hermes-home>/config.yaml`에는 `vault.root`와 `vault.writer`가 없다.
...
10. `<runtime-root>/.hermes/config.toml`에는 현재 active runtime과 다른 stale custom provider 흔적이 있다.
